...
class EmailSender:
    ...
    def select_auth_mechanism(
        ...
        if user_email is None:
            raise KeyError(
                "No variable named %s found in runtime environment."
                % app_user_email_key
            )
        ...
        if app_password is None:
            raise KeyError(
                "No variable named %s found in runtime environment." % app_password_key
            )
        else:
            return self.app_password_auth_login(user_email, app_password)

    def app_password_auth_login(
        self,
        email: str,
        app_password: str,
        ...
        mail_server = smtplib.SMTP(host=mail_server, port=mail_server_port)
        mail_server.ehlo(mail_service)
        mail_server.starttls()
        mail_server.login(user=email, password=app_password)

        logger.info("Logged into mail server.")

        self.sender_email = email

        return mail_server

    def send_mail(self, message: str, recipients: list, mail_title: str = None):
