...
class Room():
    ...
    """
    def __init__(self,
                 domain: str = puller_configs.ROOM_DOMAIN):
        """
        ...
        total_results_count = self.get_total_results_count(total_result_domain=search_domain)
    ...
    def get_total_results_count(self,
                                total_result_domain: str):
        """
        Wrapper for getting total number of results from Room Hexia API
        """
        total_results_count_resp_dict = utils.generic_get_request_wrap(total_result_domain, json.loads)
        n_total_results = total_results_count_resp_dict['_metadata']['total_search_count']
        
        return n_total_results
    

    def get_total_results(self,
                          result_search_url: str):
        """
        Wrapper for pulling all rentals from Room Hexia API
        """
        total_results_resp_dict = utils.generic_get_request_wrap(result_search_url, json.loads)
        result_rentals = total_results_resp_dict['data']

        return result_rentals
    

    def parse_rental_obj(self,
                         rental_obj: dict,
                         layer1_target_keys: list,
                         layern_target_keys: list):
        """
        ...
        for ln_key in layern_target_keys:
            ...
            if len(ln_key) == 1:
                ln_key_str = ln_key[0]
                ln_result = self.parse_rental_obj(layernm1_obj,
            ...
            # If key list is > 1, continue to dig with layer1_target_keys being None
                ...
                if len(ln_key_list_sub_keys) == 1:
                    l2_results = self.parse_rental_obj(layernm1_obj, 
    ...
    def parse_rentals(self,
        ...
        for rental_obj in self.rentals:
            ...
            truncated_info_dict = self.parse_rental_obj(rental_obj, tmp_l1_keys, tmp_ln_keys)
            ...
            renamed_info_dict = {self.db_mapping_dict[k]: v for k, v in truncated_info_dict.items()}
...
class Kamernet():
    ...
    """
    def __init__(self,
                 domain: str = puller_configs.KAMERNET_DOMAIN):
        self.site_domain = domain
        ...
        total_results_count = self.get_total_results_count(total_result_domain = search_domain,
                                                           total_result_payload = init_search_pl)
        self.rentals = self.get_total_results(result_search_url = search_domain,
    ...
    def get_total_results_count(self,
                                total_result_domain: str,
                                total_result_payload: dict):
        """
        Wrapper for getting total number of results from Kamernet .NET WebAPI
        """
        total_results_count_resp_dict = utils.generic_post_request_wrap(total_result_domain,
                                                                        handler_func = json.loads,
                                                                        pl = total_result_payload)
    ...
    def get_total_results(self,
        ...
        for page_i in range(1, n_pages_required + 1):
            ...
            page_results = utils.generic_post_request_wrap(result_search_url,
                                                           handler_func = json.loads,
                                                           pl = page_payload)
    ...
    def parse_rental_obj(self,
        ...
        fmted_url_append = url_append_fmter % (parsed_results['listingType'],
                                               parsed_results['city'],
    ...
    def parse_rentals(self):
        ...
        """
        full_parsed_rental_objs = [self.parse_rental_obj(x, puller_configs.KAMERNET_L1_KEYS) for x in self.rentals]
        key_remapped_objs = [{self.db_mapping_dict[k]: v for k, v in x.items()} for x in full_parsed_rental_objs]
...
class Pararius():
    ...
    def __init__(self,
        ...
        init_search_soup = self.selenium_soup_get(init_search_url, "search-list")
        all_listing_links = self.get_all_listing_links(init_search_soup, 
                                                       puller_configs.PARARIUS_SEARCH_URL)
        self.all_listing_links_unique = list(set(all_listing_links))

    
    def selenium_soup_get(self, 
                          url: int, 
                          test_element_class: str, 
                          load_delay: int = 10):
        """
    ...
    def get_single_page_listings(self,
                                 soup):
        """
    ...
    def get_all_listing_links(self,
                              starter_soup,
                              search_url: str):
        """
        ...
        while listing_count < total_listing_n:
            ...
            search_soup = self.selenium_soup_get(fmted_search_url, "search-list")
    ...
    def parse_rental_obj(self,
        ...
        transfer_lineitem_objs = transfer_info_obj.find_all("span", attrs = transfer_lineitem_attrs)
...
class Funda():
    ...
    def __init__(self,
        ...
        init_search_soup = self.selenium_soup_get(init_search_url, "//div[@data-test-id='search-result-item']")
        all_listing_links = self.get_all_listing_links(init_search_soup, 
                                                       puller_configs.FUNDA_SEARCH_URL)
        self.all_listing_links_unique = list(set(all_listing_links))
    ...
    def selenium_soup_get(self,
                          ...
                          load_delay: int = 30):
        """
        Function for Selenium get request awaiting load of test element, found by XPath
        """
        self.driver.get(url)
    ...
    def get_total_results_count(self,
        ...
        Extracts total listings count from generic search page
        """
        rental_count_div_attrs = {"class": "overflow-hidden text-ellipsis whitespace-nowrap font-semibold"}
    ...
    def get_all_listing_links(self,
        ...
        for page_num in range(1, page_count + 1):
            ...
            page_soup = self.selenium_soup_get(page_search_url,
                                               "//div[@data-test-id='search-result-item']")
            result_divs = page_soup.find_all("div", attrs = {"data-test-id": "search-result-item"})
            for result in result_divs:
                listing_name_obj = result.find_all("a", 
                                                   attrs = {"class": "text-blue-2 visited:text-purple-1 cursor-pointer"})[0]
    ...
    def parse_rental_obj(self,
        ...
        result_soup = self.selenium_soup_get(en_swapped_listing_link,
                                             "//span[@class='object-header__title']")
        
        # Select taal == english
        language_selector = Select(self.driver.find_element(By.XPATH,
                                                             "//select[@id='langSwitch']"))
        language_selector.select_by_visible_text("English")

        # Pull soup again with english text
        sleep(1)
        result_soup = BeautifulSoup(self.driver.page_source)
